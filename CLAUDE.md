...
**Persistence is flat JSON files under `data/`, no database.** Each persisted collection has its own `load_*()`/`save_*()` pair with the same shape: return an empty value if the file doesn't exist yet, `os.makedirs(..., exist_ok=True)` before writing.
- `src/storage.py` → `data/conversations.json` — all conversations, keyed by a UUID.
...
**Model split.** `DEFAULT_MODEL` (`qwen3:4b`, in `src/config.py`) handles chat; a separate embedding model (`nomic-embed-text`, hardcoded in `src/rag/embeddings.py`) handles RAG. `qwen3:4b` is a "thinking" model — `stream_chat` (`src/llm.py`) yields `("thinking", ...)` and `("content", ...)` tuples by reading Ollama's `message.thinking` / `message.content` fields directly, not by regex-parsing `<think>` tags out of raw text.

**No agentic tool-calling.** All tool use (web search, document search) is manually triggered by a checkbox in the UI — the model itself never decides when to call a tool. This was deliberate. An agentic version (the model deciding for itself, via Ollama's tool-calling API) was attempted and rolled back due to reliability and latency problems on local hardware; if resuming that work, treat it as a fresh design rather than assuming prior agent code exists in this repo.
